...
const TopHeader: React.FC<TopHeaderProps> = ({ view = 'scan', animate = false }) => {
  ...
  const ticketsTitleIcon = "http://localhost:3845/assets/c2d688fe42fa8efa669a1b98373859eb82f7afa7.svg"; // Reuse coupon icon as it represents a ticket/coupon

  return (
    <div className={`top-header ${animate ? 'animate-start' : ''} ${isScrolled ? 'collapsed' : ''} ${isTickets ? 'view-tickets' : ''}`}>
      <div className="header-title">
        <div className="title-icon-wrapper">
          {isTickets || isCoupons ? (
            <img src={isTickets ? ticketsTitleIcon : couponsTitleIcon} alt="" className="header-svg-icon" />
          ) : (
            <ScanIcon color="white" size={22} className="header-svg-icon" />
          )}
        </div>
        <h1 className="text-h1">{isTickets ? 'Bilety' : isCoupons ? 'Moje kupony' : 'Skaner AR'}</h1>
      </div>
